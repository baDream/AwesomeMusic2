...
public class SlidingDownPanelLayout extends ViewGroup {
    ...
    @Override
    public boolean onTouchEvent(MotionEvent ev) {       //����ٰ� �������� ��������� ���� �׳� �÷��ش�------------------------------------------------------------------------------------------------------------------------------
        if (!mCanSlide) {
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN: {
                ...
                Log.i("y", "y position " + y);
                mInitialMotionX = x;
                mInitialMotionY = y;

                //ToDo Click able!! by Jun
                //Log.e("positionY",y + "_position");
                if(y < 200) wantTouchEvents = true;
                else wantTouchEvents = false;

                if(isOpen()) wantTouchEvents = true;

                break;
            }

            case MotionEvent.ACTION_UP: {
                //if you ACTION_UP from y < 200 openPane!
                if( ev.getY() < 200) openPane();
                ...
                if (isDimmed(mSlideableView)) {
                    ...
                    if (dx * dx + dy * dy < slop * slop &&
                        ...
                        closePane(mSlideableView, 0);                                               //â �ݱ�
                        break;
                    }
                }
...
//        final int leftBound = getPaddingLeft() + lp.leftMargin;
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            //30size is over the layout!
            int maxSize;

            maxSize = getHeight() - changedView.getHeight();

            //click able
            mBackButton = (ImageView)findViewById(R.id.ic_back_button);
            ...
            mMainSoundSeekBar = (SeekBar)findViewById(R.id.mini_sound_seekBar);
...
//                mMainSoundSeekBar.setEnabled(true);
//            }
//            else{
...
//                mMainSoundSeekBar.setEnabled(false);
//            }

            //change the image color to transparency. by jun
            Drawable picture = ((ImageView)findViewById(R.id.mini_play_view_albumart)).getDrawable();
            ImageView img = (ImageView)findViewById(R.id.mini_play_view_albumart);


            ((TextView)findViewById(R.id.mini_play_view_title)).setTextColor(Color.argb(255 - (int) ((float) top / maxSize * 255), 0, 0, 0));
            ((TextView)findViewById(R.id.mini_play_view_artist)).setTextColor(Color.argb(255 - (int) ((float) top / maxSize * 255), 0, 0, 0));
            picture.setAlpha(255 - (int) ((float) top / maxSize * 255));

            if( (int) ((float) top / maxSize * 255) > 255){
                ((TextView)findViewById(R.id.mini_play_view_title)).setTextColor(Color.argb(0, 0, 0, 0));
                ((TextView)findViewById(R.id.mini_play_view_artist)).setTextColor(Color.argb(0, 0, 0, 0));
                picture.setAlpha(0);
            }


//            if(img.getDrawable().toString().contains("com.squareup.picasso.PicassoDrawable")){
//                last_img = img.getDrawable();
//            }
//            //chage the image
//            if(maxSize/3 < top){
//                picture.setAlpha((int) ((float) top / maxSize * 255));
//                img.setImageResource(R.drawable.ic_action_up);
//            }
//            else if(maxSize/3 > top){
//                picture.setAlpha(255 - (int) ((float) top / maxSize * 255 * 3));
//                if(last_img!=null) img.setImageDrawable(last_img);
//            }
//            else{
//                picture.setAlpha(0);
//            }
